...
import { deleteNews, fetchNews, updateNews, createNews } from "../../features/news/newsSlice";
...
export default function ManageNews() {
  ...
  useEffect(() => {
    if (
      location.state?.returnPath === "/admin/news" &&
      ...
      if (location.state.isEditMode && location.state.currentPostId) {
        const found = newsList.find(e => e._id === location.state.currentPostId);
        if (found) setCurrentPost(found);
      }

      setOpen(true);
      navigate("/admin/news", { replace: true, state: {} });
    }
  }, [location.state, navigate, newsList]);
  ...
  const categories = ["General", "Press Release", "Announcement", "Company"];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode && currentPost) {
        ...
        const resultAction = await dispatch(createNews(submitData));
          ...
          <DialogContent className="sm:max-w-125 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="title" name="title" value={formData.title} onChange={handleInputChange} required disabled={creating || updating} placeholder="Enter news title" />
              </div>
              ...
              <div className="space-y-2">
                ...
                <select id="category" name="category" value={formData.category} onChange={handleInputChange} required disabled={creating || updating} className="w-full border rounded-md p-2 text-sm">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-3">
                  <Input id="image" name="image" type="file" accept="image/*" onChange={handleFileChange} disabled={creating || updating} />
                  {uploadProgress > 0 && (
                    <div className="space-y-2">
              ...
              <div className="flex justify-end space-x-2 pt-4">
                <Button type="button" variant="outline" onClick={() => { setOpen(false); setUploadProgress(0); }} disabled={creating || updating}>Cancel</Button>
                <Button type="submit" disabled={creating || updating} className="min-w-24">
                  {creating || updating ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : (isEditMode ? 'Update' : 'Create')}
                </Button>
              </div>
            </form>
          </DialogContent>
        ...
        <table className="w-full">
          ...
          <tbody>
            {loading && newsList.length === 0 ? (
              ...
              <tr><td colSpan="4" className="p-8 text-center text-gray-500">No news found</td></tr>
            ) : newsList?.map((item) => (
              <tr key={item._id} className="border-b hover:bg-gray-50">
                ...
                <td className="p-4 text-gray-700">{getAuthorName(item.author)}</td>
                <td className="p-4"><span className="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded-full">{item.category}</span></td>
                <td className="p-4">
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => handleEditClick(item)} disabled={creating || updating || deletingId}><Edit className="h-3.5 w-3.5" /></Button>
                    <Button size="sm" variant="destructive" onClick={() => handleDelete(item._id)} disabled={deletingId === item._id}><Trash2 className="h-3.5 w-3.5" /></Button>
                  </div>
                </td>
              </tr>
            ))}
